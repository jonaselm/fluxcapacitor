...
After loading the data, we can create a strategy object, compute our indicators and add trading signals using the pipe operator from the `magrittr` package. Building a backtesting pipeline using the %>% operator provides a clear visualization of how data flows from raw input data through our backtest results.  
...
Once the backtest has completed, analysis can be performed either with functions within `fluxcapacitor` or through existing analytics packages through the `tidyquant` package.
...
We can quickly see that between March 1, 2006 and December 31, 2017 (our data range), our strategy would have generated a final total portfolio value of `r report_example$ledger %>% filter(row_number() == n()) %>% select(Acct_Val)` during that timeframe.
